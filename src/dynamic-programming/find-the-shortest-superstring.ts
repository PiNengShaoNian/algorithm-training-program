//这题知道了答题思路后编码难度不难，但是细节容易出错，所以写的时候要模块化的思维来写
//确保一个模块是正常的在开始下一部分代码的编写
export function shortestSuperstring(words: string[]): string {
  //该题要求包含所有单词的的子串，所以可以枚举出所有单词的可能组合，然后在将他们组合起来
  //在组合是不是简单的拼接组合，两个有重叠的部分还要去除重叠的部分比如单词[ab,bc]就可以组合为abc
  //所以我们我们要求的答案就是所有单词组合中重叠部分最长的那个组合
  ...
  //的单词是j是他们重叠部分能达到的最大长度
  const dp: number[][] = []
  //dp[i][j]只记录了重叠长度，我们还需要记录状态转移的路径，后面才能递推出答案
  //parent[i][j]表示dp[i][j]这个状态是从dp[i ^ (1 << parent[i][j])][parent[i][j]]这个
