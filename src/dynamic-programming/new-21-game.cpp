...
public:
   //未优化解法,O(k * maxPts)
    ...
    double new21Game(int n, int k, int maxPts) {
        ...
        //最终可以获得的最大的分数为maxPts + k - 1(倒数第二轮是获得了k - 1的分数，然后有选到了一个
        ...
        //每个分数的概率相同都是1 / maxPts所以dp[k - 1]就等于min(n - k + 1, maxPts) / maxPts
        dp[k - 1] = 1.0 * min(n - k + 1, maxPts) / maxPts;
        //如果 0 <= x <= k - 1
        //通过上面的未优化解法可以发现dp[x] - dp[x + 1] = (dp[x + 1] - dp[x + maxPts + 1]) / maxPts
        //所以dp[x] = dp[x + 1] - (dp[x + maxPts + 1] - dp[x + 1]) / maxPts
        for(int i = k - 2; i >= 0; --i) {
